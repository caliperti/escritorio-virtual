...
async def principal():
    ...
    async with websockets.connect(f"ws://127.0.0.1:{PORTA}/ws") as wd, \
        ...
        # o dono entra na sala e reivindica
        await wd.send(json.dumps({"tipo": "mover", "x": meio[0], "y": meio[1], "direcao": "baixo"}))
        ...
        await wv.send(json.dumps({"tipo": "mover", "x": meio[0], "y": meio[1], "direcao": "baixo"}))
        conferir("com a porta aberta a visita entra", await esperar(wv, "corrigir", limite=1.5) is None)
        await wv.send(json.dumps({"tipo": "mover", "x": 9.5 * tile, "y": 26.5 * tile, "direcao": "baixo"}))
        ...
        conferir("soltar tira o dono da sala", not z.get("dono"))
